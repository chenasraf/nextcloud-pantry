...
final class PrefsController extends OCSController {
	...
	#[NoAdminRequired]
	public function setUserPrefs(?int $lastHouseId = null, ?bool $tapRowToComplete = null, ?string $reuseExistingItems = null): DataResponse {
		return $this->runAction(function () use ($lastHouseId, $tapRowToComplete, $reuseExistingItems): DataResponse {
			$uid = $this->requireUid();
			$patch = [];
			if ($lastHouseId !== null) {
	 ...
	 * @param string|null $labelSort Label sort mode (name_asc, name_desc, custom).
	...
	public function setHousePrefs(
		...
		return $this->runAction(function () use ($houseId, $imageFolder, $photoSort, $photoFoldersFirst, $noteSort, $checklistItemSort, $checklistSort, $categorySort, $labelSort, $storeSort, $showAddedBy, $lastCurrency, $notifyPhoto, $notifyNoteCreate, $notifyNoteEdit, $notifyItemAdd, $notifyItemRecur, $notifyItemDone): DataResponse {
			$uid = $this->requireUid();
			$this->auth->requireMember($houseId, $uid);
			$patch = array_filter([
