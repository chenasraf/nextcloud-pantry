...
class LabelMapper extends QBMapper {
	public function __construct(IDBConnection $db) {
		parent::__construct($db, Application::tableName('labels'), Label::class);
	}
	 ...
	 * labels yet — so a caller can assign `max + 1` to append a new label at the
	...
	/**
	 * Find a label by name within a single scope. The scope is a house plus a
	 * list: a null $listId matches the house-global scope, a set $listId matches
	 * that list's own scope. Used to enforce per-scope name uniqueness.
	 */
	public function findByHouseListAndName(int $houseId, ?int $listId, string $name): ?Label {
		$qb = $this->db->getQueryBuilder();
		$qb->select('*')
			->from($this->getTableName())
			->where($qb->expr()->eq('house_id', $qb->createNamedParameter($houseId, IQueryBuilder::PARAM_INT)))
	...
	/**
	 * Delete every label scoped to the given list. Global labels (null list_id)
	 * are untouched. Used when a list is permanently removed.
	 */
