---
title: Permissions
description: How Pantry scopes everything to a house, the role and capability model, and exactly which capability each action requires.
sidebar:
  order: 2
---
...
what a member may do inside that house is decided by the **roles** they hold. This page explains
...
| `canEditFields`   | Creating, editing, reordering, and deleting custom-field definitions                              |
...
<Aside type="note">
	There is no separate "edit item" or "archive item" capability. Editing an item, reordering,
	...
	`canEditFields` covers custom-field **definitions**, not the values stored on items. Filling in
	an item's custom-field values travels with the item write itself — `canAddItems` when creating
	the item, `canEditLists` when editing it.
</Aside>
...
Roles are edited in the **Roles** dialog, opened from **Manage roles** on the
[Manage house](/docs/administration/manage-house) page. It is **admin-only**. Each role is a card
showing its **name** and a type chip — **Built-in admin**, **Built-in member**, or **Custom**.

- **Edit what a role can do** — expand **Customize** on the Member role or a custom role to reveal
  its capability switches, grouped into **Checklists**, **Checklist items**, **Photos** and
  **Notes**. Each group's **View** switch gates the rest of the group: turn View off and the other
  switches in that group turn off and grey out. The **Admin** role is locked to every permission and
  has no switches.
- **Add a role** — **Add role** creates a custom role with every permission granted, ready to rename
  and pare back. **Duplicate role** copies an existing role's permissions into a new custom one.
- **Delete a role** — only custom roles can be deleted; the built-in Admin and Member roles cannot.
  A pending delete is marked and can be undone until you save.
- **Save** — nothing takes effect until you select **Save changes**. Closing with unsaved edits asks
  you to confirm first.

Roles are assigned to people separately, in the **Members** dialog (**Manage members** on the same
page) — each member holds one or more roles, and their effective permissions are the union across
them.

To scope a single list rather than a whole role, restrict that list to roles, or share an individual
entity with specific people — both described under [Per-list access and sharing](#per-list-access-and-sharing)
above.
...
<Aside type="note">Restricting a list to specific roles is admin-only.</Aside>
...
Columns: **View** = `canViewLists`, **Manage** = `canEditFields`.

| Action                                                      | View | Manage |
| ----------------------------------------------------------- | :--: | :----: |
| List a house's custom-field definitions                     |  ✅  |        |
| Create a field definition                                   |      |   ✅   |
| Edit a field definition (name, list scope)                  |      |   ✅   |
| Reorder field definitions                                   |      |   ✅   |
| Delete a field definition                                   |      |   ✅   |
| Delete a `select` option (remapping or clearing its values) |      |   ✅   |

<Aside type="note">
	Setting custom-field values on an item is not listed here — it is part of the item write and
	requires `canAddItems` or `canEditLists` accordingly.
</Aside>
